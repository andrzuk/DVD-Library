...
class Operator
{
	...
	public function Tree($id, $item, $params, $access, $acl)
	{
		$this->content_title = $params['content_title'];
		...
		if (in_array($this->user_status, $access) && $acl) // są uprawnienia
		{
			...
			// wyświetla formularz wypełniony danymi:
			$this->site_content = $this->view_object->ShowTree($this->record_object, $item, $this->records_list);
		}
		else // brak uprawnień
		{
			$this->AccessDenied();
	...
	public function Folder($id, $params, $access, $acl)
	{
		$this->content_title = $params['content_title'];
		...
		if (in_array($this->user_status, $access) && $acl) // są uprawnienia
		{
			...
			// pobiera rekord o danym Id:
			$this->record_object = $this->model_object->GetFolder($id);
			
			// wyświetla formularz wypełniony danymi:
			$this->site_content = $this->view_object->ShowFolder($this->record_object, $this->list_columns);
		}
		else // brak uprawnień
		{
			$this->AccessDenied();
	...
	public function FoundList($params, $access, $acl)
	{
		...
		$search_kind = isset($params['search_range']['kind']) ? $params['search_range']['kind'] : NULL;
		$search_type = isset($params['search_range']['type']) ? $params['search_range']['type'] : NULL;
		
		$search_caption = empty($search_text) ? '(wszystkie)' : $search_text;
		$search_kind_caption = empty($search_kind) ? NULL : ' ('.$search_kind.')';
		$search_type_caption = empty($search_type) ? NULL : ' ('.$search_type.')';
		
		$this->content_title .= ' - Wyniki dla: "' . $search_caption . '"' . $search_kind_caption . $search_type_caption;
		...
		if (in_array($this->user_status, $access) && $acl) // są uprawnienia
		{
			...
			// pobiera listę rekordów:
			$record_list = $this->model_object->Search($search_text, $params['search_range'], $restrict_id, $this->db_params);
	...
	public function FoundFolders($params, $access, $acl)
	{
		$this->content_title = $params['content_title'];
		...
		if (in_array($this->user_status, $access) && $acl) // są uprawnienia
		{
			...
			// wyświetla listę rekordów:
			$this->site_content = $this->view_object->ShowFolders($record_list, $this->list_columns, $this->list_params);
		}
		else // brak uprawnień
		{
			$this->AccessDenied();
