...
class Navi
{
	...
	public function init($list_columns)
	{
		...
		foreach ($list_columns as $key => $value)
		{
			...
			if (!empty($db_name)) $db_fields[] = $db_name;
		}
	...
	public function update($model_object, &$list_params)
	{
		...
		// ograniczenia dla typu:
		$restrict_range = isset($this->search_domain) ? $this->search_domain : NULL;

		// ograniczenia dla usera:
		$restrict_id = isset($this->restrict) ? $this->restrict : NULL;

		// oblicza paginację:
		$model_object->SetPages($restrict_value, $restrict_range, $restrict_id, $this->display_rows);
