...
class FormBuilder
{
	...
	public function build_form()
	{
		...
		{
			$main_text .= '<tr class="DataRow">';
			foreach ($v as $key => $value)
			{
				if ($key == 'caption')
				{
					$main_text .= '<td class="DataCell"> '. $value .': </td>';
				}
				else if ($key == 'data' || $key == 'caption_data')
				{
					switch ($key)
					{
						case 'data':
							...
							break;
						case 'caption_data':
							$main_text .= '<td colspan="'. $this->columns .'" class="DataCell">';						
							break;
					}
					foreach ($value as $arr_key => $arr_val)
					{
						...
						else if ($arr_val['type'] == 'textarea')
						{
							...
							if ($this->editor === 'true') // CkEditor włączony
							{
								...
								$main_text .= '</tr>';
								$main_text .= '<tr class="DataRow">';
								$main_text .= '<td class="DataCell" colspan="'. $this->columns .'">';
							}
							$main_text .= '<textarea ';
							foreach ($arr_val as $atr_key => $atr_value)
							{
