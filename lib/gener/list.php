...
class ListBuilder
{
				...
				if (is_array($row))
				{
					...
					foreach ($row as $key => $value)
					{
						if ($this->attribs[$idx]['visible']) // kolumna widoczna
						{
							...
							{
								$main_text .= '<a href="index.php?route=category&id=' . $row['category_id'] . '">';
								$main_text .= $this->get_split_text(strip_tags($value), 10);
								$main_text .= '</a>';
							}
	...
	/*
	 * Lista wyszukiwania płyt:
	 */
	 
	public function build_discs_list()
	{
		$main_text = NULL;
				...
				if (is_array($row))
				{
					...
					foreach ($row as $key => $value)
					{
						if ($this->attribs[$idx]['visible']) // kolumna widoczna
						{
							...
							$main_text .= '<a href="index.php?route=discs&action=details&id=' . $row['file_id'] . '">';
							$main_text .= $this->get_split_text(strip_tags($value), 10);
							$main_text .= '</a>';
	...
	public function build_folders_list()
	{
		$main_text = NULL;
				...
				if (is_array($row))
				{
					...
					foreach ($row as $key => $value)
					{
						if ($this->attribs[$idx]['visible']) // kolumna widoczna
						{
							...
							$main_text .= '<a href="index.php?route=discs&action=folder&id=' . $row['folder_id'] . '">';
							$main_text .= $this->get_split_text(strip_tags($value), 10);
							$main_text .= '</a>';
